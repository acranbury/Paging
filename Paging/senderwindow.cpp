...
void SenderWindow::SendText()
{
    ...
    if (ui->headerChk->isChecked()){
        ...
        if ((QString::compare("huffman", ui->compressCmb->itemText(ui->compressCmb->currentIndex()), Qt::CaseInsensitive)) == 0)
        {
            if(!(textBuf = (char*)malloc(sizeof(char)* numChars)))
            {
                QMessageBox::information(NULL, "Error!", "Error malloccing outBuf");
            }
